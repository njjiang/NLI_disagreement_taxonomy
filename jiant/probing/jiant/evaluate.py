...
from jiant.tasks.tasks import (
    ...
    GLUEDiagnosticTask,
    NLI4wayTask,
    NLIDisagreementTaxonomyTask,
    NLI7wayTask,
    NLIMultiLabelClassificationTask,
)
...
def evaluate(
    ...
    for task in tasks:
        ...
        for batch_idx, batch in enumerate(generator):
            ...
            n_class = out["logits"].shape[-1]
            for i in range(n_class):
                cols[f"logit_{i}"] = out["logits"][:,i].cpu().detach().numpy().tolist()
...
def write_preds(
    ...
    for task in tasks:
        ...
        elif isinstance(task, NLI4wayTask):
            _write_NLI4way_preds(task, preds_df, pred_dir, split_name)
        elif isinstance(task, NLIDisagreementTaxonomyTask):
            _write_NLIDisagreementTaxonomy_preds(task, preds_df, pred_dir, split_name)
        elif isinstance(task, NLI7wayTask):
            _write_NLI7way_preds(task, preds_df, pred_dir, split_name)
        elif isinstance(task, NLIMultiLabelClassificationTask):
            _write_NLIMultilabel_preds(task, preds_df, pred_dir, split_name)
        else:
...
def _write_NLI4way_preds(
    task: str,
    preds_df: pd.DataFrame,
    pred_dir: str,
    split_name: str,
):
    trg_map = ["n", "e", "c", "complicated"  ]
    preds_file = "%s_%s.jsonl" % (task.name, split_name)
    preds_file = os.path.join(pred_dir, preds_file)
    with open(preds_file, "w", encoding="utf-8") as preds_fh:
        for row_idx, row in preds_df.iterrows():
            row["preds"] = trg_map[row["preds"]]
            row["labels"] = trg_map[row["labels"]]
            out_d = row.to_dict()
            preds_fh.write("{0}\n".format(json.dumps(out_d)))

def _write_NLI7way_preds(
            task: str,
            preds_df: pd.DataFrame,
            pred_dir: str,
            split_name: str,
    ):
        trg_map = ['n', 'e', 'c', 'en', 'ec', 'nc', 'enc']
        preds_file = "%s_%s.jsonl" % (task.name, split_name)
        preds_file = os.path.join(pred_dir, preds_file)
        with open(preds_file, "w", encoding="utf-8") as preds_fh:
            for row_idx, row in preds_df.iterrows():
...
def _write_NLIMultilabel_preds(
        task: str,
        preds_df: pd.DataFrame,
        pred_dir: str,
        split_name: str,
    ...
    with open(preds_file, "w", encoding="utf-8") as preds_fh:
        for row_idx, row in preds_df.iterrows():
            row["preds"] = [label for label, i in zip(trg_map, row["preds"]) if i == 1]
            row["labels"] = [label for label, i in zip(trg_map, row["labels"]) if i == 1]
            out_d = row.to_dict()
            preds_fh.write("{0}\n".format(json.dumps(out_d)))



def _write_NLIDisagreementTaxonomy_preds(
        task: str,
            preds_df: pd.DataFrame,
            pred_dir: str,
            split_name: str,
        ...
        trg_map = ["n", "c", "e",
         ...
         'imperfection',
         'coref',
         'minimal underspecified content',
         'temporal reference',
         'Question H',
         'shuffle NP',
         'implicature',
