...
from jiant.allennlp_mods.fbeta_multi_label_measure import F1MultiLabelMeasure 
...
def process_single_pair_task_split(
    ...
    def _make_instance(input1, input2, labels, idx):
        ...
        if classification:
            ...
            d["labels"] = NumericField(labels, label_namespace=label_namespace)
        else:
...
# take all the augmented data (lots of ENC)
@register_task("augmented_4way_MNLI", rel_path="augmented_4way_MNLI/")
@register_task("augmented_4way_MNLI_SNLI", rel_path="augmented_4way_MNLI_SNLI/")
@register_task("augmented_4way_MNLI_matched", rel_path="augmented_4way_MNLI_matched/")
# downsample ENC, class balanced
@register_task("downsample_augmented_4way_MNLI", rel_path="downsample_augmented_4way_MNLI/")
@register_task("downsample_augmented_4way_MNLI_SNLI", rel_path="downsample_augmented_4way_MNLI_SNLI/")
@register_task("downsample_augmented_4way_MNLI_matched", rel_path="downsample_augmented_4way_MNLI_matched/")
# remove all augmented complicated, only chaosNLI complicated, augment ENC, class balanced
@register_task("downsample_augment3_MNLI", rel_path="downsample_augment3_MNLI/")
@register_task("downsample_augment3_MNLI_SNLI", rel_path="downsample_augment3_MNLI_SNLI/")
@register_task("downsample_augment3_MNLI_matched", rel_path="downsample_augment3_MNLI_matched/")
# chaosNLI only, very imbalanced a lot of complicate
@register_task("4way_chaos_MNLI_SNLI", rel_path="4way_chaos_MNLI_SNLI/")
@register_task("4way_chaos_MNLI_matched", rel_path="4way_chaos_MNLI_matched/")
# MNLI/SNLI only based on 5 anotations
@register_task("4way_MNLI", rel_path="4way_MNLI/")
@register_task("4way_MNLI_SNLI", rel_path="4way_MNLI_SNLI/")
@register_task("4way_MNLI_matched", rel_path="4way_MNLI_matched/")
@register_task("MNLI_matched", rel_path="MNLI_matched/")
@register_task("MNLI_mismatched", rel_path="MNLI_mismatched/")

# Two ended up in the paper
@register_task("chaos", rel_path="chaos_multilabel/") # == 4way_chaos_MNLI_matched
@register_task("chaos_orig", rel_path="chaos_orig_multilabel/") # == downsample_augmented_4way_MNLI
@register_task("chaos_orig_full", rel_path="chaos_orig_full_multilabel/") # == downsample_augmented_4way_MNLI
class NLI4wayTask(PairClassificationTask):

    def __init__(self, path, max_seq_len, name, **kw):
        """ Do stuff """
        super(NLI4wayTask, self).__init__(name, n_classes=4, **kw)
        self.path = path
        self.max_seq_len = max_seq_len
        ...
        self.scorer3 = F1Measure(1)
        self.scorer4 = F1Measure(2)
        self.scorer5 = F1Measure(3)
        self.scorers = [self.scorer1, self.scorer2, self.scorer3, self.scorer4, self.scorer5]
        self.val_metric = "%s_f1" % name

    def load_data(self):
        """ Process the dataset located at path.  """
        targ_map = {"n": 0, "e": 1, "c": 2, "complicated":3, 
                # For MNLI
                "neutral":0, "entailment":1, "contradiction": 2, "-":3,
                # For multilabel: basically map all the multiple labels to complicated
                "en": 3, "nc": 3, "ec": 3, "enc": 3,
                }
        def _load_jsonl(data_file):
            data = [json.loads(d) for d in open(data_file, encoding="utf-8")]
            sent1s, sent2s, trgs, idxs = [], [], [], []
            p_key = "sentence1" if self.name.startswith("MNLI") else "premise"
            h_key = "sentence2" if self.name.startswith("MNLI") else "hypothesis"
            for idx, example in enumerate(data):
                sent1s.append(
                    tokenize_and_truncate(
                        self._tokenizer_name, example[p_key], self.max_seq_len
                    )
                )
                sent2s.append(
                    tokenize_and_truncate(
                        self._tokenizer_name, example[h_key], self.max_seq_len
                    )
                )
                trg = targ_map[example["gold_label"]]
                trgs.append(trg)
        ...
        log.info("\tFinished loading 4wayNLI data.")
    ...
    def get_metrics(self, reset=False):
        ...
        pcs2, rcl2, f12 = self.scorer3.get_metric(reset)
        pcs3, rcl3, f13 = self.scorer4.get_metric(reset)
        pcs4, rcl4, f14 = self.scorer5.get_metric(reset)
        pcs = (pcs1 + pcs2 + pcs3 + pcs4) / 4
        rcl = (rcl1 + rcl2 + rcl3 + rcl4) / 4
        f1 = (f11 + f12 + f13 + f14) / 4
        return {"accuracy": acc, "f1": f1, "precision": pcs, "recall": rcl, "complicated_f1": f14}


@register_task("disagreement_taxonomy", rel_path="disagreement_taxonomy/")
class NLIDisagreementTaxonomyTask(PairClassificationTask):

    def __init__(self, path, max_seq_len, name, **kw):
        """ Do stuff """
        super(NLIDisagreementTaxonomyTask, self).__init__(name, n_classes=14, **kw)
        self.path = path
        self.max_seq_len = max_seq_len
        ...
        self.scorer2 = FBetaMeasure(average="macro")
        self.scorers = [self.scorer1, self.scorer2]
        self.val_metric = "%s_accuracy" % name
    ...
    def load_data(self):
        ...
        targ_map = {
            ...
            'imperfection': 6,
            'coref': 7,
            'minimal underspecified content': 8,
            'temporal reference': 9,
            'Question H': 10,
            'shuffle NP': 11,
            'implicature': 12,
            'syntactic': 13
    ...
    def get_metrics(self, reset=False):
        """Get metrics specific to the task.
            ...
            - scorers{2,3,4} compute class-specific F1,
                and we macro-average to get multi-class F1"""
        acc = self.scorer1.get_metric(reset)
        f1_dict = self.scorer2.get_metric(reset)
        return {"accuracy": acc,
                "f1": f1_dict["fscore"], "precision": f1_dict["precision"], "recall": f1_dict["recall"]}


@register_task("chaos_multilabel_7way", rel_path="chaos_multilabel_7way/")
class NLI7wayTask(PairClassificationTask):

    def __init__(self, path, max_seq_len, name, **kw):
        """ Do stuff """
        super(NLI7wayTask, self).__init__(name, n_classes=7, **kw)
        self.path = path
        self.max_seq_len = max_seq_len
    ...
    def load_data(self):
        """ Process the dataset located at path.  """
        targ_map = {'n': 0, 'e': 1, 'c': 2, 'en': 3, 'ec': 4, 'nc': 5, 'enc': 6}
...
@register_task("chaos_multilabel", rel_path="chaos_multilabel/")
@register_task("chaos_orig_multilabel", rel_path="chaos_orig_multilabel/")
@register_task("chaos_orig_full_multilabel", rel_path="chaos_orig_full_multilabel/")
@register_task("MNLI_matched_multilabel", rel_path="MNLI_matched/")
@register_task("MNLI_mismatched_multilabel", rel_path="MNLI_mismatched/")
class NLIMultiLabelClassificationTask(PairClassificationTask):

    def __init__(self, path, max_seq_len, name, **kw):
        """ Do stuff """
        super(NLIMultiLabelClassificationTask, self).__init__(name, n_classes=3, **kw)
        self.path = path
        self.max_seq_len = max_seq_len
        ...
        self.scorer2 = F1MultiLabelMeasure(average="macro")
        self.scorers = [self.scorer1, self.scorer2]
        self.val_metric = "%s_f1" % self.name
        self.val_metric_decreases = False
    ...
    def load_data(self):
        ...
        def _load_jsonl(data_file):
            ...
            for idx, example in enumerate(data):
                ...
                trg = [1 if x in example["gold_label"] else 0 for x in targ_map]
                #trg = ["e" in example["gold_label"], "n" in example["gold_label"], "c" in example["gold_label"]]
                trgs.append(trg)
                idxs.append(idx)
            return [sent1s, sent2s, trgs, idxs]
    ...
    def update_metrics(self, out, batch):
        ...
        self.scorer1(preds, labels)
        # multilabel f1
        self.scorer2(out["probs"], out["labels"])
