...
class MultiTaskModel(nn.Module):
    ...
    def _pair_sentence_forward(self, batch, task, predict):
        ...
        if "labels" in batch:
            ...
            if isinstance(task, NLIMultiLabelClassificationTask):
                # numeric fields introduce one more dim: [batch_size, 1, 3], squeeze it out here
                labels = labels.squeeze(1) if len(labels.size()) > 2 else labels
                out["loss"] = F.binary_cross_entropy_with_logits(logits, labels)
                labels = labels.detach()
                logits = logits.detach()
                probs = torch.sigmoid(logits)
                out["probs"] = probs
                out["preds"] = torch.gt(out["probs"], 0.5).float()
            else:
                out["loss"] = F.cross_entropy(logits, labels)
            out["labels"] = labels
