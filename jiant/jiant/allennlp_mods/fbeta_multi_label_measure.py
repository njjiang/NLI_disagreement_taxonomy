...
class FBetaMultiLabelMeasure(FBetaMeasure):
    ...
    def __call__(
        ...
        if mask is None:
            mask = torch.ones_like(gold_labels, dtype=torch.uint8)
        ...
        true_positives = (gold_labels * threshold_predictions).type(torch.uint8) & mask & pred_mask
        ...
        pred_bins = class_indices[threshold_predictions.type(torch.uint8) & mask & pred_mask]
        # Watch it:
        ...
        gold_labels_bins = class_indices[gold_labels.type(torch.uint8) & mask]
